...
def set_to_model(extended_code_mapping, site_cd, k=3):
    ...
    for data in extended_code_mapping: 
        ...
        if set_result:
            mapping_set_tmp = pd.DataFrame(set_result, columns=[c.name for c in cursor.description])
        ...
        extended_code_mapping_tmp = list({json.dumps(d, sort_keys=True): d for d in extended_code_mapping_tmp.copy()}.values())
    
    filtered_mapping_set = mapping_set
            
    if not filtered_mapping_set.empty:
        extended_code_mapping_tmp = [d for d in extended_code_mapping_tmp if d.get('extended_info')[0] in filtered_mapping_set['cstrt_model_code'].tolist()]
...
def specification_check(
    ...
    if country_code == "GB":
        mapping_set, extended_code_mapping_new = set_to_model(extended_code_mapping, site_cd)
        if not mapping_set.empty:
            ...
            set_info_list = list(
                uk_product_spec_basics.objects.filter(
                    ...
                    site_cd=site_cd,
                ).values("category_lv1", "category_lv2", "category_lv3", "display_name", "model_code", "launch_date")
            )
            
            if set_info_list:
                set_identified_model = pd.DataFrame(set_info_list)
                set_identified_model.columns = ["Category Lv1", "Category Lv2","Category Lv3", "Model Name","Model Code", "Release Date"]
                set_identified_model["Release Date"] = set_identified_model["Release Date"].apply(
                    lambda x: x.strftime("%Y-%m") if x is not None else x
                )
                component_codes = mapping_set.groupby('model_code')['cstrt_model_code'].apply(lambda x: ', '.join(x)).reset_index()
                component_codes.columns = ['Model Code', 'Component Model Codes']
                set_identified_model = pd.merge(set_identified_model, component_codes, on='Model Code', how='left')
    ...
    n_item = 3 if len(extended_code_mapping_new) == 1  or len(unique_model_code) <= 3 else 1
        ...
        for data in extended_code_mapping_new:
            ...
            if (
                ...
                if not bundle_df.empty and country_code == "KR":
                    df_identified_model_tmp = df_identified_model[df_identified_model['Model Code'].isin(bundle_df['Model Code'].tolist())]
                    if 'Component Model Code' in df_identified_model_tmp.columns:
                        df_identified_model_tmp = df_identified_model_tmp.drop(['Component Model Code'],axis=1)
                    ...
                    if df_identified_model_tmp.empty and country_code =="KR":
                        ...
                        tmp_result = list(
                            product_category.objects.filter(
                                ...
                                site_cd=site_cd,
                            ).values("product_category_lv1","product_category_lv2","product_category_lv3","goods_nm", "mdl_code","release_date")
                        )
                        if tmp_result:
                            df_identified_model_tmp = pd.DataFrame(tmp_result)
                            df_identified_model_tmp.columns = ["Category Lv1", "Category Lv2", "Category Lv3", "Model Name", "Model Code", "Release Date"]
                            df_identified_model_tmp = pd.merge(
                                df_identified_model_tmp, bundle_df, on="Model Code"
                            )
                    df_identified_model = pd.concat([df_identified_model, df_identified_model_tmp], axis=0)
                    grouped_bundle_spec_table = grouped_bundle_spec_table.fillna("-").reset_index()
...
if __name__ == "__main__":
    ...
    #       {
    #         "mapping_code": "Series 8 AI Energy Washing Machine and Heat Pump Tumble Dryer, 9kg, Black",
    ...
    #       },
    #       {
    #         "mapping_code": "Series 6 AI Energy Washing Machine, 11kg and Series 7 Tumble Dryer with OptimalDry, White",
    #         "category_lv1": "Home Appliances",
    #         "category_lv2": "Washers and Dryers",
    #         "category_lv3": "Washing Machines",
    #         "edge": "set",
    ...
    extended_code_mapping = [
          {
            "mapping_code": "2025 Neo QLED QNF90 (138cm) 풀 모션 슬림핏 벽걸이형",
            "category_lv1": "Flat Panel Displayer",
            "category_lv2": "QLED TV",
            "category_lv3": "Neo QLED",
            "edge": "recommend",
            "meta": "",
            "extended_info": [
              "KQ55QNF90-W"
          ...
          {
            "mapping_code": "2023 QLED 4K QC68 (214 cm)",
            "category_lv1": "Flat Panel Displayer",
            "category_lv2": "QLED TV",
            "category_lv3": "QLED",
            "edge": "recommend",
    ...
    print(
        specification_check(
            extended_code_mapping, country_code, is_pd_intelligence=True
        )[0]
    )
