<template>
  <v-dialog v-model="dialogVisible" persistent :max-width="alphaModel.dialogWidth? alphaModel.dialogWidth:'600px'">
    <template v-slot:activator="{ props: activatorProps }">
      <v-btn v-bind="activatorProps" :variant="props.editType === 'EDIT' ? 'outlined' : 'tonal'" :color="props.editType === 'EDIT' ? 'primary' : 'default'" :size="props.editType === 'EDIT' ? 'x-small' : 'small'" :class="props.editType === 'EDIT' ? '' : 'ml-2'" @click="props.editType === 'EDIT' ? showDialogValues() : initDialogValues()">{{ btnValue }}
        <v-tooltip
      ...
      <v-card :title="dialogTitle">
          ...
          <template v-for="dialog in alphaModel.dialog">
            ...
            <v-row>
              <v-col
                ...
                :key="dialog.index"
              >
              <div class="text-caption pl-2 pb-1"> {{ dialog.label }}</div>
              <codemirror
                v-model="dialog.selected"
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            variant="tonal"
            class="my-2"
            @click="props.editType === 'EDIT' ? update('update') : create()"
          ></v-btn>
          <v-btn
            v-if = "props.editType === 'EDIT'"
