<template>
  ...
  <div v-if="envStore.isMobile" :class="locationClass" style="width: 100%;">
  <v-card
      :color="getComputedStyle('chatCardText')"
      flat style="width: 100%;"
      :class="!isBot ? 'ml-12':'ml-0'"
      :style="getComputedStyle('chatCardBackground')"
  >
  <v-card-text v-if="isBot" class="pt-2 pl-4 pb-0 mb-n3">
    <div class="text-caption"> Model: {{ model }} | Message ID: {{ messageId }}</div>
  </v-card-text>
  <v-card-text>
    <v-row>
      <v-col>
        <Blinker v-if="isBot && loadingMessage !== undefined" class="mt-3 pl-0"><div>{{props.loadingMessage}}</div></Blinker>
        <div class="renderedHtml py-0" v-html="renderHtml(content)"></div>
        <v-row v-if="isBot && completed" class="ml-0 mt-n2 pb-2">
          ...
          <v-dialog max-width="600" v-model="dialogReference">
            <template v-slot:activator="{ props: activatorProps }">
              <v-btn v-bind="activatorProps" variant="text" size="x-small" icon="mdi-text-box-search-outline" class="text-grey ml-n1" @click="checkReference()"></v-btn>
            </template>

            <template v-slot:default="{ isActive }">
              <v-card title="Referenced information">
                <v-card-text class="pb-0">
                </v-card-text>
...
<!-- ##### Below code is for PC ############################################################# -->
  <!-- <div v-if="!envStore.isMobile" :class="locationClass" style="width: 100%;"> -->
  <div v-else :class="locationClass" style="width: 100%;">
  <v-card
      :color="getComputedStyle('chatCardText')"
      flat style="width: 90%;"
      :style="getComputedStyle('chatCardBackground')"
  ...
  <v-card-text>
    <v-row>
      <v-col v-if="isBot" cols="2">
        <v-container>
          <v-row class="d-flex justify-end ">
            <img class="mt-3" src="@/assets/img/alpha-chat.png" style="width: 20px; height: 20px">
          </v-row>
        </v-container>
      </v-col>
      <v-col cols="6">
...
<!-- ##### Below code is for PC ############################################################# -->
        ...
        <v-row v-if="isBot && completed" class="ml-0 mt-02 pb-2">
          <v-btn variant="text" size="x-small" icon="mdi-thumb-up-outline" :class="thumbUpClass" @click="thumbUpClick()"></v-btn>
            ...
            <template v-slot:default="{ isActive }">
              <v-card title="Please tell me in more detail.">
                <v-card-text class="pb-0">
                  <v-checkbox density="compact" hide-details
                    v-model="checkList[0]"
                    label="&nbsp;Wrong product spec or price, and so on. (제품 스펙이나 가격 등, 틀린 정보가 포함됨)"
                  ></v-checkbox>
                  <v-checkbox density="compact" hide-details
                    v-model="checkList[1]"
                    label="&nbsp;Wrong explanation or image included (잘못된 설명이나 이미지가 포함됨)"
                  ></v-checkbox>
                  <v-checkbox density="compact" hide-details
                    v-model="checkList[2]"
                    label="&nbsp;Answer that is off-topic (질문의 의도에 맞지 않는 답변임)"
                  ></v-checkbox>
                  <v-checkbox density="compact" hide-details
                    v-model="checkList[3]"
                    label="&nbsp;Not wrong, but lacks or overflows with information (틀린 답은 아니나, 정보가 불충분 또는 과잉 )"
                  ></v-checkbox>
                  <v-checkbox density="compact" hide-details
                    v-model="checkList[4]"
                    label="&nbsp;On-topic, but outdated. (질문의 의도에 맞지만, 최신 정보가 아님"
                  ></v-checkbox>
                  <v-checkbox density="compact" hide-details
                    v-model="checkList[5]"
                    label="&nbsp;Something else (기타 의견)"
                  ></v-checkbox>
                  <v-textarea
                    v-shortkey.avoid
                    v-model="comment"
  ...
  if (component === 'chatCardBackground') {
    ...
    if (themeStore.darkMode) {
      if (props.isBot) {
        ...
        return 'background-color: #242424'
      }
    } else {
      if (props.isBot) {
...
const locationClass = computed(() => {
  return props.isBot ? 'd-flex justify-start pa-3' : 'd-flex justify-end pa-3'
})
