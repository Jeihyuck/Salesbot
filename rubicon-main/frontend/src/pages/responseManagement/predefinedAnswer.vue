<template class="mr-n2">
    <v-col cols="12"  style="height: calc(100vh - 72px); overflow-y: auto;">
      <alphaDataTableView :alphaModel="alphaModel" ref="appraisalCheckTable">
        <template v-slot:predefined_query_custom="{ item }">
          <codemirror
            v-model="item.predefined_query"
...
const alphaModel = reactive({
  ...
  itemName: 'Predefined Answer',
  paging: { page: 1, itemsPerPage: 15 },
  function: rubiconAdmin.predefined.table,
  crud: [true, true, true, true],
  headerId: '4328e0e4-5e5a-4773-92c9-11b51b0a3cbe',
  customFields: [],
  filter: {
    hideBoarder: false,
    country_code: {
    ...
    category: {
      ...
      type: 'dropdown',
      clearable: true,
      selector: ['recommendation', 'malicious_attack', 'conversation', 'VD'],
      col: 2,
      selected: null
    },
    embeddingSearch: {
  ...
  dialog: {
    ...
    category: {
      ...
      type: 'dropdown',
      required: true,
      selector: ['recommendation', 'malicious_attack', 'conversation'],
      selected: ''
    },
    query: {
      label: 'Query',
